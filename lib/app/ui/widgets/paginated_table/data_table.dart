...
/// Signature for [DataColumnCustom.onSort] callback.
typedef DataColumnCustomSortCallback = void Function(
    int columnIndex, bool ascending);

/// Column configuration for a [DataTableCustom].
///
/// One column configuration must be provided for each column to
/// display in the table. The list of [DataColumnCustom] objects is passed
/// as the `columns` argument to the [DataTableCustom] constructor.
@immutable
class DataColumnCustom {
  /// Creates the configuration for a column of a [DataTableCustom].
  ///
  ...
  ///
  /// See [DataTableCustom.sortColumnIndex] and [DataTableCustom.sortAscending].
  final DataColumnCustomSortCallback? onSort;

  bool get _debugInteractive => onSort != null;
}

/// Row configuration and cell data for a [DataTableCustom].
///
/// One row configuration must be provided for each row to
/// display in the table. The list of [DataRowCustom] objects is passed
/// as the `rows` argument to the [DataTableCustom.new] constructor.
///
/// The data for this row of the table is provided in the [cells]
/// property of the [DataRowCustom] object.
@immutable
class DataRowCustom {
  /// Creates the configuration for a row of a [DataTableCustom].
  ///
  ...
  ///
  /// If a [DataCellCustom] in the row has its [DataCellCustom.onTap] callback defined,
  ...
  ///
  /// If a [DataCellCustom] in the row has its [DataCellCustom.onTap], [DataCellCustom.onDoubleTap],
  /// [DataCellCustom.onLongPress], [DataCellCustom.onTapCancel] or [DataCellCustom.onTapDown] callback defined,
  /// that callback behavior overrides the gesture behavior of the row for
  /// that particular cell.
  ...
  bool get _debugInteractive =>
      ...
      cells.any((DataCellCustom cell) => cell._debugInteractive);
}

/// The data for a cell of a [DataTableCustom].
///
/// One list of [DataCellCustom] objects must be provided for each [DataRowCustom]
/// in the [DataTableCustom], in the new [DataRowCustom] constructor's `cells`
/// argument.
@immutable
class DataCellCustom {
  /// Creates an object to hold the data for a cell in a [DataTableCustom].
  ///
  ...
  static const DataCellCustom empty = DataCellCustom(SizedBox.shrink());
  ...
  /// [DataRowCustom.onSelectChanged] is provided).
  ...
  /// row (if [DataRowCustom.onSelectChanged] is provided).
...
/// [PaginatedDataTableCustom] which automatically splits the data into
/// multiple pages.
///
/// {@tool dartpad}
/// This sample shows how to display a [DataTableCustom] with three columns: name, age, and
/// role. The columns are defined by three [DataColumnCustom] objects. The table
/// contains three rows of data for three example users, the data for which
/// is defined by three [DataRowCustom] objects.
///
...
///
/// {@tool dartpad}
/// This sample shows how to display a [DataTableCustom] with alternate colors per
...
/// See also:
///
///  * [DataColumnCustom], which describes a column in the data table.
///  * [DataRowCustom], which contains the data for a row in the data table.
///  * [DataCellCustom], which contains the data for a single cell in the data table.
///  * [PaginatedDataTableCustom], which shows part of the data in a data table and
///    provides controls for paging through the remainder of the data.
///  * <https://material.io/design/components/data-tables.html>
class DataTableCustom extends StatelessWidget {
  /// Creates a widget describing a data table.
  ///
  /// The [columns] argument must be a list of as many [DataColumnCustom]
  ...
  ///
  /// The [rows] argument must be a list of as many [DataRowCustom] objects
  ...
  /// not be null.
  ///
  /// Each [DataRowCustom] object in [rows] must have as many [DataCellCustom]
  /// objects in the [DataRowCustom.cells] list as the table has columns.
  ///
  ...
  DataTableCustom({
    ...
    this.clipBehavior = Clip.none,
        ...
        assert(rows != null),
        assert(!rows
            .any((DataRowCustom row) => row.cells.length != columns.length)),
        assert(dividerThickness == null || dividerThickness >= 0),
        assert(clipBehavior != null),
        _onlyTextColumn = _initOnlyTextColumn(columns);

  /// The configuration and labels for the columns in the table.
  final List<DataColumnCustom> columns;
  ...
  ///
  /// If this is null, then the [DataRowCustom.onSelectChanged] callback of
  /// every row in the table is invoked appropriately instead.
  ///
  /// To control whether a particular row is selectable or not, see
  /// [DataRowCustom.onSelectChanged]. This callback is only relevant if any
  /// row is selectable.
  ...
  /// selectable. However, if [DataRowCustom.onSelectChanged] is not set for any row,
  ...
  static int? _initOnlyTextColumn(List<DataColumnCustom> columns) {
    int? result;
    for (int index = 0; index < columns.length; index += 1) {
      final DataColumnCustom column = columns[index];
      if (!column.numeric) {
        if (result != null) {
          return null;
        }
  ...
  bool get _debugInteractive {
    return columns.any((DataColumnCustom column) => column._debugInteractive) ||
        rows.any((DataRowCustom row) => row._debugInteractive);
  }
  ...
  void _handleSelectAll(bool? checked, bool someChecked) {
    ...
    if (onSelectAll != null) {
      ...
      for (final DataRowCustom row in rows) {
  ...
  Widget build(BuildContext context) {
    ...
    final bool anyRowSelectable =
        rows.any((DataRowCustom row) => row.onSelectChanged != null);
    final bool displayCheckboxColumn = showCheckboxColumn && anyRowSelectable;
    final Iterable<DataRowCustom> rowsWithCheckbox = displayCheckboxColumn
        ? rows.where((DataRowCustom row) => row.onSelectChanged != null)
        : <DataRowCustom>[];
    final Iterable<DataRowCustom> rowsChecked =
        rowsWithCheckbox.where((DataRowCustom row) => row.selected);
    ...
    for (int dataColumnIndex = 0;
        ...
        dataColumnIndex += 1) {
      final DataColumnCustom column = columns[dataColumnIndex];
      ...
      rowIndex = 1;
      for (final DataRowCustom row in rows) {
        final DataCellCustom cell = row.cells[dataColumnIndex];
...
///    [DataRowCustom.onSelectChanged] is defined and [DataCell.onTap]
///    is not.
