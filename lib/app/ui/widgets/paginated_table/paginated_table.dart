...
///
/// Data is read lazily from a [DataTableSourceCustom]. The widget is presented
/// as a [Card].
///
/// See also:
///
...
class PaginatedDataTableCustom extends StatefulWidget {
  /// Creates a widget describing a paginated [DataTableCustom] on a [Card].
  ///
  /// The [header] should give the card's header, typically a [Text] widget.
  ///
  /// The [columns] argument must be a list of as many [DataColumnCustom] objects as
  ...
  /// [DataTableSourceCustom]. The same source should be provided each time a
  /// particular [PaginatedDataTableCustom] widget is created; avoid creating a new
  /// [DataTableSourceCustom] with each new instance of the [PaginatedDataTableCustom]
  ...
  /// [CardTheme], since a [Card] wraps the inner [DataTableCustom].
  ...
  final DataTableSourceCustom source;
...
class PaginatedDataTableCustomState extends State<PaginatedDataTableCustom> {
  ...
  final Map<int, DataRowCustom?> _rows = <int, DataRowCustom?>{};
  ...
  DataRowCustom _getBlankRowFor(int index) {
    return DataRowCustom.byIndex(
      index: index,
      cells: widget.columns
          .map<DataCellCustom>(
              (DataColumnCustom column) => DataCellCustom.empty)
          .toList(),
    );
  }

  DataRowCustom _getProgressIndicatorRowFor(int index) {
    bool haveProgressIndicator = false;
    final List<DataCellCustom> cells =
        widget.columns.map<DataCellCustom>((DataColumnCustom column) {
      if (!column.numeric) {
        haveProgressIndicator = true;
        return const DataCellCustom(CircularProgressIndicator());
      }
      return DataCellCustom.empty;
    }).toList();
    if (!haveProgressIndicator) {
      haveProgressIndicator = true;
      cells[0] = const DataCellCustom(CircularProgressIndicator());
    }
    return DataRowCustom.byIndex(
      index: index,
      cells: cells,
  ...
  List<DataRowCustom> _getRows(int firstRowIndex, int rowsPerPage) {
    final List<DataRowCustom> result = <DataRowCustom>[];
    final int nextPageFirstRowIndex = firstRowIndex + rowsPerPage;
    bool haveProgressIndicator = false;
    for (int index = firstRowIndex; index < nextPageFirstRowIndex; index += 1) {
      DataRowCustom? row;
      if (index < _rowCount || _rowCountApproximate) {
        row = _rows.putIfAbsent(index, () => widget.source.getRows(index));
